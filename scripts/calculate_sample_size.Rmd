...
It indicated an overall sample size (approximately 1,500), and the direction that the vendor should use a sampling strategy that stratifies by sector, region of the province, voluntary/compulsory trade, and completion/cancellation status.  

This report outlines the methods used to determine a potential sample size and sampling strategy for the RFS. 
It finds that sampling 10% of the eligible apprentices from each stratum - that is, 10% from each of the 32 possible combinations of sector, region, and status - will lead to 95% confidence and a margin of error of 3%.
...
The projected sample sizes of each of the 32 strata are listed in Table
\@ref(tab:all-strata)
in Appendix A below.
...
## Appendix A: Detailed Stratum Projections

Table
\@ref(tab:all-strata)
shows the projected number of apprentices with each combination of region, sector, and completion status. 
It also includes the sample sizes used in calculating the total of 1,144 apprentices.


```{r all-strata}
strata_with_mins %>%
  arrange(region, sector, status) %>%
  select(
    Region = region,
    Sector = sector, 
    Status = status, 
    `Projected Apprentices` = projected_stratum_total,
    `Sample Size` = sample_first_min_10
  ) %>%
  mutate(`Sample Size` = round(`Sample Size`)) %>%
  adorn_totals() %>%
  # filter(`Sample Size` < 10) %>%
  knitr::kable(
    caption = "Projected Sample Sizes for All Strata"
  )
```

### Appendix B: Breakdown by Completion Status

Table \@ref(tab:by-status) below shows exits by cancellation/completion status.

```{r by-status}
projected_monthly_clients %>%
  # filter(fiscal_year == "2021-22YTD") %>%
  # group_by(status) %>%
  group_by(fiscal_year, status) %>%
  summarize(
    ytd_clients = sum(clients),
    projected_clients = sum(projected_clients)
  ) %>%
  adorn_totals() %>%
  knitr::kable(
    caption = "Exits by Cancellation/Completion Status across Fiscal Years"
  )


```

### Appendix C: Stratification Without Sector

Because the sample size was estimated *before* stratification, removing sector as a stratification variable will have no effective impact on the recommendation of sampling strategy (10% of each stratum) or the estimated sample size (1,100 - 1,200 completed surveys). 
Projections of the population and sample from each stratum are listed in Table
\@ref(tab:no-sector)
below.
...
strata_totals <- 
  ...
  mutate(
    projected_sample_size = round(0.10 * projected_stratum_total)
  )
...
strata_totals %>%
  ...
  adorn_totals() %>%
  knitr::kable(
    caption = "Projected Strata and 10% Sample Sizes Without Sector"
  )
```
